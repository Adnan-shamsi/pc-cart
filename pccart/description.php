...
if(isset($_GET['submit']) && $_SESSION['customer_id'])
{
   ...
   $res = mysqli_query($conn,"SELECT * FROM reviews
                              WHERE cus_id = {$_SESSION['customer_id']}
   ...
   if (mysqli_num_rows($res) == 0)
   {
     mysqli_query($conn,"INSERT INTO `reviews` (`comment`, `cus_id`, `title`, `p_id`)
                         VALUES
                         ('{$comment}',{$_SESSION['customer_id']},{$title},{$pid})") or die("unsuccessful35");
   }
   else
   {
     mysqli_query($conn,"UPDATE `reviews`
                         SET `comment`='{$comment}', `title`={$title}
                         WHERE cus_id = {$_SESSION['customer_id']}
                         AND p_id ={$pid}") or die("unsuccessful");
   }
...
<div class="container" style='margin-top:100px'>
  <div class="row ">
    ...
    <div class="col-md-6 mx-2">
      ...
      <div class="row mt-4">
        <button class='btn btn-success buyBtn m-2'   value="<?php echo $row['Product_id'] ?>">Buy Now</button>
        <button class='btn btn-warning cartBtn m-2' value="<?php echo $row['Product_id'] ?>" style='float:right'>Add To <i class="fa fa-cart-plus" aria-hidden="true"></i></button>
      </div>
...
?>
        ...
        <div class="col-lg-3 col-md-4 col-sm-6 similar col-5 my-1 ">
          ...
          <a  href="<?php echo 'description.php?pid='. $row2['Product_id'] ?>"> <img class="img-fluid img-onsmall" src="<?php echo 'admin/'. $prod_image_location .$row2['first_image']?>" alt="Card image cap" style="height:20vh">
            ...
            <div class='pb-2 pt-1 pb-lg-1 mx-2'>
              <button class='btn btn-success buyBtn'  value="<?php echo $row2['Product_id'] ?>"> Buy Now</button>
              <button class='btn btn-warning cartBtn' value="<?php echo $row2['Product_id'] ?>" style='float:right'><i class="fa fa-cart-plus" aria-hidden="true"></i></button>
            </div>
          </div>
       </div>
  ...
  while($row3 = mysqli_fetch_array($result3)) {
   ?>
    <div class='row my-2 '>
        <div class="col-md-4">
          <h4 class='text-info pr-2'> <?php echo $row3['FirstName'] .' ' . $row3['LastName']?> </h4>
        </div>
        <div class="col-md-8">
  ...
  <!--review ends-->
    ...
    <button type="submit" class="btn btn-primary" name='submit'  <?php if(!isset($_SESSION['customer_id'])) echo 'disabled'  ?> >Submit</button>
  </form>
